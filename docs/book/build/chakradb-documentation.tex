...
\item
  ...
  \href{../architecture/query-router.md}{The Query Layer}.)
\item
  ...
  thousand rows, not a billion. (See
  \href{../architecture/mvcc.md}{MVCC}.)
\item
...
\href{../architecture/overview.md}{Architecture}, shows how those pieces
fit, and why the ``buy execution, build storage'' split is the whole
strategy.
...
\href{storage-engine.md}{The Storage Engine}.
...
table size. This is the \href{pk-index.md}{primary-key index}.
\end{quote}
...
\item
  ...
  range cannot overlap. This is \href{pruning.md}{zonemap pruning}.
\item
...
\item
  ...
  exists. See \href{pk-index.md}{The Primary-Key Index}.
\end{itemize}
...
\href{../operations/limits.md}{Limits}).
...
\chapter{Pk Index}\label{pk-index}

\begin{quote}
\textbf{This chapter is being written.} The section is scaffolded in the
table of contents; the full text lands in an upcoming revision. See the
neighbouring chapters and \texttt{docs/requirements.md} for the current
material.
\end{quote}
...
writers. This chapter is the \emph{model}; the
\href{../algorithms/visibility.md}{visibility algorithm} and the
\href{../algorithms/gc-watermark.md}{GC watermark} get the full
...
\begin{center}\includegraphics[width=0.92\linewidth,height=0.80\textheight,keepaspectratio]{diagrams/diagram-6.pdf}\end{center}
...
\href{../algorithms/gc-watermark.md}{The GC Watermark}.
...
chapter is the shape; the \href{../algorithms/wal.md}{WAL algorithm} and
\href{../algorithms/recovery.md}{recovery} chapters have the byte-level
detail.
...
See \href{checkpointing.md}{Checkpointing}.

\begin{center}\includegraphics[width=0.92\linewidth,height=0.80\textheight,keepaspectratio]{diagrams/diagram-8.pdf}\end{center}
...
\begin{center}\includegraphics[width=0.92\linewidth,height=0.80\textheight,keepaspectratio]{diagrams/diagram-9.pdf}\end{center}
...
\href{io.md}{The I/O Abstraction}) --- a real POSIX backend for
...
\chapter{Checkpointing}\label{checkpointing}
...
\chapter{Compaction}\label{compaction}
...
selective ranges that \href{pruning.md}{zonemap pruning} makes cheaper
...
\href{transactions.md}{Transactions}.
...
\href{../comparisons/duckdb.md}{DuckDB comparison} shows where each
lands.

\chapter{Pruning}\label{pruning}
...
\chapter{Transactions}\label{transactions}
...
\begin{center}\includegraphics[width=0.92\linewidth,height=0.80\textheight,keepaspectratio]{diagrams/diagram-11.pdf}\end{center}
...
\href{../algorithms/gc-watermark.md}{GC watermark} back, so the versions
it may read are not compacted away). Every statement in the transaction
reads that one consistent instant, extended by its own writes. Other
connections' commits are invisible.
...
\chapter{How to Read This Part}\label{how-to-read-this-part}

\epigraph{An algorithm must be seen to be believed.}{--- Donald Knuth}

This part is the engine at the level of its algorithms. Where Part II
described the \emph{shape} of ChakraDB --- the tiers, the clock, the log
--- this part gives the \emph{procedures}, each with pseudocode, a
complexity, and, where correctness is subtle, a \textbf{proposition and
a proof sketch}.

\section{Conventions}\label{conventions-1}

\begin{itemize}
\tightlist
\item
  \textbf{Algorithms are numbered globally} --- \texttt{ALGORITHM\ 1},
  \texttt{ALGORITHM\ 2}, \ldots{} --- so they can be referenced across
  chapters. Each states its \texttt{Input}, its \texttt{Output}, and
  annotates lines with \texttt{▷} comments on the right.
\item
  Assignment is written \texttt{←}. Comparison and set notation
  (\texttt{≤}, \texttt{∈}, \texttt{∩}, \texttt{∞}) mean the usual
  things.
\item
  \textbf{Propositions} capture the guarantees that are easy to get
  wrong (visibility, crash-atomicity, GC safety). Proof \emph{sketches}
  are arguments, not machine-checked proofs; where a full proof is
  future work, the text says so.
\item
  \texttt{V}, \texttt{E}, \texttt{n} are sizes; \texttt{α} is the
  inverse-Ackermann function (effectively constant); CSN is a commit
  sequence number.
\end{itemize}

\section{The map of this part}\label{the-map-of-this-part}
...
\bottomrule\noalign{}
\endlastfoot
\href{key-index.md}{The Sorted-Part Key Index} & 1--2 & How is a row
found in \texttt{O(log\ n)} with no key→location map? \\
\href{visibility.md}{MVCC Visibility} & 3--4 & Which version does a
snapshot see, and why is cold data free? \\
\href{wal.md}{Write-Ahead Logging \& Group Commit} & 5--6 & How is a
write durable before it is acknowledged? \\
\href{recovery.md}{Crash Recovery} & 7 & How is the exact acknowledged
state rebuilt after a crash? \\
\href{merge.md}{The Merge / Compaction Algorithm} & 8--9 & How are parts
merged and dead versions reclaimed? \\
\href{gc-watermark.md}{The GC Watermark} & 10 & How does compaction
never reclaim a version a reader can see? \\
\href{pruning.md}{Zonemap Part Pruning} & 11 & How does a scan skip
parts that cannot match? \\
\href{routing.md}{Query Routing} & 12 & How is each statement sent to
the right engine? \\
\href{decimal.md}{Exact Decimal Arithmetic} & 13 & How is
\texttt{DECIMAL} exact, never \texttt{f64}? \\
\href{temporal.md}{Temporal Encoding} & 14 & How are
\texttt{DATE}/\texttt{TIMESTAMP} stored and rendered? \\
\href{bloom.md}{Bloom Filters \& the Lookup Funnel} & 15 & How is a part
excluded without touching disk? \\
\end{longtable}

\section{The recurring theme}\label{the-recurring-theme}

Nearly every algorithm here spends structure at \emph{write} time so
that a \emph{read} is cheap and lock-free: parts are sorted so lookups
binary-search; versions are stamped so visibility is a comparison;
zonemaps are precomputed so scans prune. The
\href{../introduction/cost-model.md}{cost model} is the philosophy; this
part is where it is cashed out.

\chapter{The Sorted-Part Key Index}\label{the-sorted-part-key-index}
...
the scaling ceiling (see \href{../operations/limits.md}{Limits}).

\chapter{MVCC Visibility}\label{mvcc-visibility}
...
transactions, and the \href{../graph/csr.md}{graph CSR} consistently.

\chapter{Write-Ahead Logging \& Group
...
state --- is \href{recovery.md}{the next chapter}.

\chapter{Crash Recovery}\label{crash-recovery}
...
\href{../guide/sql-reference.md}{the SQL surface}.
...
mode (\href{wal.md}{ALGORITHM 5}), one whose frame and covering
...
\href{visibility.md}{visibility}. The floor makes recovered CSNs safe:
the next allocation is strictly greater than anything on disk.

\chapter{The Merge / Compaction
...
\href{gc-watermark.md}{GC horizon}.
...
(the \href{gc-watermark.md}{GC watermark}). A row can be physically
dropped only when no snapshot \texttt{≥\ H} could still see it:
...
is the silent-wrong-answer hazard the \href{gc-watermark.md}{GC
watermark} chapter is devoted to.
...
the cheap \href{visibility.md}{full-part fast path} for future scans.
...
\href{../guide/observability.md}{\texttt{Storage::stats()}}. This is the
``cost of fast writes is paid in compaction, and made visible''
commitment from the \href{../introduction/cost-model.md}{cost model}.
...
(\href{visibility.md}{ALGORITHM 3}). Every row visible to \texttt{S} has
...
\chapter{The GC Watermark \& Live-Snapshot
...
\chapter{Zonemap Part Pruning}\label{zonemap-part-pruning}
...
\href{../graph/adjacency.md}{graph adjacency}.
...
here rather than to a full vectorized scan (see \href{routing.md}{Query
Routing}).
...
\href{../graph/adjacency.md}{clustered adjacency} --- the graph
traversal primitive falls directly out of ALGORITHM 11.
...
\chapter{Query Routing (Cost Model)}\label{query-routing-cost-model}
...
\item
  ...
  appends to the WAL; it owns the \href{visibility.md}{snapshot clock}.
\item
...
\item
  \textbf{Point lookup (line 8).} The \href{key-index.md}{index funnel}
  is \texttt{O(log\ n)}; a vectorized scan operator would be far slower
  for a single row.
\item
  \textbf{Selective range (lines 9--10).} \href{pruning.md}{Zonemap
...
part, the \texttt{excludes} test (\href{pruning.md}{ALGORITHM 11}) plus
...
--- the \href{gc-watermark.md}{GC watermark}). Transient reads at the
current clock need no pin; long reads (a full scan, a DataFusion query,
a transaction) hold one.
...
\chapter{Temporal Encoding}\label{temporal-encoding}
...
prunes via \href{pruning.md}{zonemaps}, and a date can be a primary key.
...
\chapter{Bloom Filters \& the Lookup
...
\href{key-index.md}{point-lookup funnel}, after the min/max bounds and
...
B/row resident index budget (\href{key-index.md}{Proposition 1}): the
...
\href{routing.md}{router} exploits.
...
\item
  ...
  This is the \href{../architecture/mvcc.md}{concurrency wedge} applied
  to graphs --- the thing pure graph libraries (a dead static copy) and
  lock-based graph databases cannot do in one embedded process. See
  \href{live-analytics.md}{Live Graph Analytics}.
\item
  ...
  Arrow columns --- no sort, no hash join. See \href{csr.md}{The CSR
  Snapshot}.
\end{enumerate}
...
remove the key-encoding step. See \href{modeling.md}{Modeling a Property
Graph}.
\end{quote}

\chapter{Modeling a Property Graph}\label{modeling-a-property-graph}
...
typed columns. They get \href{../algorithms/pruning.md}{zonemaps}, so a
...
\href{csr.md}{CSR builder} symmetrize for undirected algorithms
...
The next chapters --- \href{adjacency.md}{Clustered Adjacency} and
\href{csr.md}{The CSR Snapshot} --- show why this simple mapping
traverses fast.

\chapter{Clustered Adjacency}\label{clustered-adjacency}
...
This is the same \href{../algorithms/pruning.md}{zonemap part pruning}
that accelerates a selective SQL \texttt{WHERE} --- reused as the graph
...
\item
  ...
  CSR; see \href{csr.md}{The CSR Snapshot}.)
\end{itemize}
...
\href{../operations/limits.md}{resident index}: ``fits in a big server's
...
immutable \href{csr.md}{CSR snapshot}. Because the view is a consistent
copy taken from one MVCC snapshot, every algorithm below runs
...
\item
  ...
  range.
\item
  \texttt{out\_neighbors(node)} --- a single \href{adjacency.md}{pruned
...
\bottomrule\noalign{}
\endlastfoot
BFS / shortest path & reachability, hops & \texttt{O(V+E)} & CSR \\
PageRank & influence, ranking & \texttt{O(iters·E)} & CSR \\
Connected components & clustering, islands & \texttt{O(E·α(V))} &
union-find \\
Triangle counting & density, communities & \texttt{O(E\^{}\{1.5\})} &
sorted adjacency \\
Out-degree / neighbors & centrality, expansion & \texttt{O(d)} & range
scan \\
\end{longtable}

Deliberately \textbf{not} in v1 (expensive or hard --- offered later or
via export): exact betweenness centrality (all-pairs), Louvain
modularity, exact diameter, general subgraph isomorphism. See the
\href{../../graph-exploration.md}{design exploration} for the roadmap.

\chapter{Live Graph Analytics}\label{live-graph-analytics}
...
share nothing but the append-only clock. The
\href{../algorithms/gc-watermark.md}{GC watermark} holds the snapshot's
versions alive for the algorithm's duration even as newer data is
compacted.
...
\href{../graph/overview.md}{Part IV --- Graph} is available in
\emph{both} profiles.
\end{quote}
...
directory instead --- see \href{first-rust.md}{Your First Database
(Rust)}.
...
The Python driver implements \href{../guide/python-driver.md}{PEP 249
(DB-API 2.0)} and exposes the graph engine via \texttt{conn.graph(name)}
--- see \href{graph.md}{A Graph in Five Minutes}.
...
\chapter{Your First Database (Rust)}\label{your-first-database-rust}
...
\href{../reference/rust-api.md}{\texttt{Outcome}};
\texttt{SqlEngine::query} is a convenience that returns rows already
rendered as
...
binary floating point --- see \href{../algorithms/decimal.md}{Exact
Decimal Arithmetic}.
...
transaction sees a consistent \href{../architecture/mvcc.md}{MVCC
...
\href{../architecture/durability.md}{Durability} and
\href{../algorithms/recovery.md}{Crash Recovery}.
...
\tightlist
\item
  \href{python.md}{Python in Five Minutes} --- the same engine from
  Python.
\item
  \href{graph.md}{A Graph in Five Minutes} --- treat your tables as a
  graph.
\item
  \href{../guide/sql-reference.md}{The SQL Reference} --- the full
  statement surface.
\end{itemize}

\chapter{Python in Five Minutes}\label{python-in-five-minutes}
...
surface is exposed through a standard
\href{../guide/python-driver.md}{PEP 249 (DB-API 2.0)} driver, so if you
have used \texttt{sqlite3} you already know the shape of it.
...
\texttt{rollback}, and the full \href{../reference/errors.md}{exception
...
keyed by node id. The full catalogue is in \href{graph.md}{A Graph in
Five Minutes} and \href{../graph/algorithms.md}{Graph Algorithms}.
...
\chapter{A Graph in Five Minutes}\label{a-graph-in-five-minutes}
...
\href{../architecture/mvcc.md}{MVCC snapshot} of that table. This
chapter is the whirlwind tour.
...
\href{../graph/csr.md}{CSR} snapshot. Take it once and a whole pipeline
sees one coherent graph, even while writers keep appending edges:
...
default cascade) \\
\textbf{Similarity} & \texttt{common\_neighbors},
\texttt{jaccard\_similarity} \\
...
\part{Developer Guide \& Tutorials}

\chapter{Sql Reference}\label{sql-reference}
...
\chapter{Transactions}\label{transactions-2}
...
\chapter{Observability}\label{observability}
...
\chapter{Graph App}\label{graph-app}
...
\chapter{Python Driver}\label{python-driver}
...
\chapter{Durability}\label{durability}
...
(\href{../graph/modeling.md}{The Graph Model}).
...
(\href{../graph/live-analytics.md}{Live Graph Analytics}) --- so every
...
\item
  ...
  under peak load (\href{../guide/observability.md}{Observability});
  back up the store on a schedule (\href{../guide/backup.md}{Backup \&
  Restore}).
\end{itemize}
...
\begin{center}\includegraphics[width=0.92\linewidth,height=0.80\textheight,keepaspectratio]{diagrams/diagram-32.pdf}\end{center}
...
(\href{../algorithms/decimal.md}{Exact Decimal Arithmetic}).
...
The whole risk engine is one
\href{../architecture/overview.md}{materialized worker} --- a named,
incrementally-maintained derivation of the data. In Rust it implements
\texttt{MaterializedWorker} and is registered on the change stream:
...
\item
  ...
  expensive ring analysis (see \href{../graph/live-analytics.md}{Live
  Graph Analytics}).
\item
  Watch \texttt{Storage::stats()} for checkpoint lag and backpressure
  under peak ingest (see
  \href{../guide/observability.md}{Observability}).
\item
  Back up the store with \texttt{backup\_to} on a cadence (see
  \href{../guide/backup.md}{Backup \& Restore}).
\end{itemize}
...
the numbers and method are in \href{methodology.md}{Benchmark
Methodology}.
...
\chapter{Postgres}\label{postgres}
...
\chapter{Rust Api}\label{rust-api}
...
\chapter{Python Api}\label{python-api}
