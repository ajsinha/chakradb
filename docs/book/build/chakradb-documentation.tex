...
\item
  ...
  (\emph{Getting Started}) and the Part VI (\emph{Case Studies}).
\item
  \textbf{Architects} evaluating the engine --- read Part I
  (\emph{Introduction}), Part II (\emph{The Engine}), and Part VII
  (\emph{Perspective}).
\item
  \textbf{Contributors and the curious} who want to know \emph{how} it
  works down to the algorithm --- Parts II--IV carry the pseudocode and
  complexity.
\end{itemize}
...
II --- The Engine & Storage (parts, index, compaction, pruning), MVCC \&
transactions, durability, the HTAP query router, and exact types --- the
design \emph{and} the algorithms, together. \\
III --- The Graph Engine & The property-graph layer: clustered
adjacency, CSR snapshots, and the full graph-algorithm library. \\
IV --- Reactive & The change stream (CDC), materialized workers and the
registry, and the sink transports --- how applications react to
committed data. \\
V --- Getting Started & Install, first database, Python, and a first
graph --- Rust and Python. \\
VI --- Case Studies & Worked end-to-end applications: real-time AML,
counterparty \& market risk, recommendations, and fraud. \\
VII --- Perspective & Head-to-head with the alternatives, and where
ChakraDB fits. \\
\end{longtable}
...
\subsection{\ldots and reactive}\label{and-reactive}

A passive database answers questions. ChakraDB also \textbf{reacts}. A
committed-change stream (CDC) publishes every
\texttt{INSERT}/\texttt{UPDATE}/\texttt{DELETE} after it commits --- off
the write path, so the writer never waits --- and \textbf{materialized
workers} fold that stream into derived state: a running aggregate, a
live graph projection, a set of alerts, maintained incrementally and
queryable at any time. This closes the loop --- \emph{store → analyze →
react → store} --- on one consistent live dataset, in one process. It is
what turns the three flagship applications built in Part VI ---
real-time \href{../case-studies/aml.md}{AML},
\href{../case-studies/ccr.md}{counterparty \& market risk}, and
\href{../case-studies/reco.md}{recommendations} --- from queries-you-run
into systems-that-run, each reacting to 150+ million events per hour
while ingest never pauses.
...
\item
  ...
  performance claim in this book ships with the code that produced it. A
  number without a reproducible source is a hypothesis, not a
  measurement.
\end{enumerate}
...
reads see a merged view across the tiers. This chapter covers the store
and the algorithms that run over it --- the sorted-part key index,
merge/compaction, zonemap pruning, and Bloom filters --- each in its own
section below.
...
\item
  ...
  stamps --- trading write amplification for scan speed. See Compaction.
\end{enumerate}
...
\item
  ...
  range cannot overlap. This is zonemap pruning.
\item
...
\item
  ...
  exists. See The Primary-Key Index.
\end{itemize}
...
\href{../introduction/introduction.md}{cost model}.
...
The Bloom test is the second filter in the point-lookup funnel, after
the min/max bounds and before the binary search:
...
B/row resident index budget (Proposition 1): the Bloom bits, the min/max
bounds, the version stamps, and the deletion vector together stay flat
with table size, which is what lets the engine keep data on disk and the
index in memory.
...
writers. This chapter is the \emph{model}; the visibility algorithm and
the GC watermark get the full treatment in Part III.
...
full mechanism --- and why it is correct under concurrency --- is in The
GC Watermark.
...
GC watermark back, so the versions it may read are not compacted away).
Every statement in the transaction reads that one consistent instant,
extended by its own writes. Other connections' commits are invisible.
...
chapter covers all three --- the design, then the WAL and recovery
...
mode (ALGORITHM 5), one whose frame and covering \texttt{fsync}
completed --- so its frame is intact and precedes the torn tail, and it
is replayed (or already in a part below the checkpoint). An
\emph{unacknowledged} write either never reached the log or sits in the
torn tail; the CRC rejects the latter, so it is dropped. Hence the
recovered set is exactly the acknowledged set. This is stress-tested by
tens of thousands of randomized crash trials across all durability modes
and schemas (\texttt{crash\_consistency}, \texttt{durable\_sql\_crash}).
∎
\end{quote}
...
remove the key-encoding step. See Modeling a Property Graph.
\end{quote}
...
The sections below --- clustered adjacency, and then
...
shape stays effectively \textbf{O(1) as the table grows 100×} For a
graph, that means neighbor lookup cost tracks the node's degree, not the
graph's size.
...
\texttt{personalized\_pagerank} is the same iteration with the teleport
vector concentrated on a \textbf{seed set} instead of uniform --- the
engine behind risk propagation (AML/CCR) and the \texttt{recommend} walk
(see below).
...
(\texttt{strongly\_connected\_components}, and
\texttt{laundering\_cycles} for the non-trivial ones) find
\emph{directed} cycles --- the core of AML round-trip detection.
...
\href{../../../graph-exploration.md}{design exploration} for the
roadmap.
...
\href{../graph/model.md}{the Graph Engine part} is available in
\emph{both} profiles.
\end{quote}
...
directory instead --- see Your First Database (Rust).
...
The Python driver implements PEP 249 (DB-API 2.0) and exposes the graph
engine via \texttt{conn.graph(name)} --- see A Graph in Five Minutes.
...
\texttt{Outcome}; \texttt{SqlEngine::query} is a convenience that
returns rows already rendered as
...
surface is exposed through a standard PEP 249 (DB-API 2.0) driver, so if
you have used \texttt{sqlite3} you already know the shape of it.
...
\texttt{rollback}, and the full exception hierarchy
(\texttt{IntegrityError}, \texttt{ProgrammingError},
...
keyed by node id. The full catalogue is in A Graph in Five Minutes and
...
the numbers and method are in Benchmark Methodology.
